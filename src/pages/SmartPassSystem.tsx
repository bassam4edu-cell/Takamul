...
const SmartPassSystem = () => {
  const [view, setView] = useState<'agent' | 'teacher' | 'tracker'>('agent');
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="fixed top-6 left-6 z-50 flex flex-col gap-3">
        ...
        <button 
          onClick={() => setView(view === 'tracker' ? 'agent' : 'tracker')}
          className="flex items-center gap-3 bg-white px-6 py-3 rounded-2xl border border-slate-200 shadow-xl hover:shadow-2xl transition-all font-black text-slate-700 group"
        >
          <UserCheck className="w-5 h-5 text-emerald-600 group-hover:scale-110 transition-transform duration-500" />
          {view === 'tracker' ? 'العودة لواجهة الوكيل' : 'نظام تتبع حالة الطالب'}
        </button>
      </div>
