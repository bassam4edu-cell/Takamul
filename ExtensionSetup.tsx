...
const VPRadar: React.FC = () => {
  ...
  const [radarMode, setRadarMode] = useState<'all' | 'specific' | null>(null);
  ...
  useEffect(() => {
    // Read from localStorage first for immediate sync
    const localConfig = localStorage.getItem('radar_config');
    if (localConfig) {
      try {
        const parsed = JSON.parse(localConfig);
        setRadarMode(parsed.type);
        if (parsed.active_period) setRadarPeriod(parsed.active_period);
    ...
    apiFetch('/api/settings')
      ...
      .then(data => {
        ...
        if (!localConfig && data.radar_mode) {
          setRadarMode(data.radar_mode as 'all' | 'specific');
          setIsRadarActive(true);
          if (data.radar_period) {
            setRadarPeriod(Number(data.radar_period));
          }
        }
      })
      .catch(console.error);
  ...
  const handleActivateRadar = async () => {
    ...
    const config = { 
      type: radarMode, 
      active_period: radarMode === 'specific' ? radarPeriod : null 
    };
    localStorage.setItem('radar_config', JSON.stringify(config));
    ...
    try {
      const res = await apiFetch('/api/settings', {
        ...
        body: JSON.stringify({
          settings: {
            ...
            radar_period: radarMode === 'specific' ? radarPeriod.toString() : '1'
          }
        })
      });
      if (res.ok) {
      ...
      console.error('Failed to activate radar', err);
    } finally {
      setIsActivating(false);
    }
  };

  const handleDeactivateRadar = async () => {
    // Remove from localStorage
    localStorage.removeItem('radar_config');
    ...
    try {
      ...
      console.error('Failed to deactivate radar', err);
    }
  };
    ...
    return (
      <div className="max-w-3xl mx-auto p-4 md:p-8 mt-10">
        <div className="bg-white rounded-3xl shadow-xl border border-slate-100 p-8 md:p-12 text-center space-y-8 relative overflow-hidden">
          ...
          <div className="inline-flex items-center justify-center w-24 h-24 rounded-full bg-primary/10 text-primary mb-4 relative">
            <div className="absolute inset-0 rounded-full border-4 border-primary/20 animate-ping"></div>
            <Zap size={48} className="relative z-10" />
          </div>
          
          <div className="space-y-3">
            <h1 className="text-3xl md:text-4xl font-black text-slate-800">بوابة رادار التحضير</h1>
            <p className="text-slate-500 text-lg max-w-lg mx-auto">حدد صلاحيات التحضير للمعلمين قبل تفعيل النظام</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-right mt-8">
            <button
              onClick={() => setRadarMode('all')}
              className={`p-6 rounded-2xl border-2 transition-all duration-200 flex flex-col gap-4 ${
                radarMode === 'all' 
            ...
            >
              <div className={`w-12 h-12 rounded-full flex items-center justify-center ${radarMode === 'all' ? 'bg-primary text-white' : 'bg-slate-100 text-slate-500'}`}>
                <Unlock size={24} />
              </div>
              <div>
                <h3 className="text-xl font-bold text-slate-800 mb-1">السماح لجميع الحصص</h3>
                <p className="text-sm text-slate-500">يمكن للمعلم اختيار أي حصة وتحضيرها بحرية تامة.</p>
              </div>
            </button>
            ...
            >
              <div className={`w-12 h-12 rounded-full flex items-center justify-center ${radarMode === 'specific' ? 'bg-primary text-white' : 'bg-slate-100 text-slate-500'}`}>
                <Lock size={24} />
              </div>
              <div>
                <h3 className="text-xl font-bold text-slate-800 mb-1">تحضير حصة محددة فقط</h3>
                <p className="text-sm text-slate-500">تقييد المعلمين بتحضير الحصة التي تحددها أنت فقط.</p>
              </div>
            </button>
          </div>
          ...
          <AnimatePresence>
            {radarMode === 'specific' && (
              <motion.div
                ...
                className="pt-6"
              >
                <div className="max-w-xs mx-auto text-right space-y-2">
                  <label className="block text-sm font-bold text-slate-700">اختر الحصة المسموح بها:</label>
                  <select
                    value={radarPeriod}
                    onChange={(e) => setRadarPeriod(Number(e.target.value))}
                    className="w-full bg-slate-50 border-2 border-slate-200 rounded-xl py-3 px-4 text-lg font-bold text-slate-800 focus:ring-4 focus:ring-primary/20 focus:border-primary outline-none transition-all"
                  >
                    {[1, 2, 3, 4, 5, 6, 7].map(p => (
          ...
          <div className="pt-8 border-t border-slate-100">
            <button
              ...
              disabled={!radarMode || isActivating}
              className="w-full md:w-auto px-12 py-4 bg-slate-900 hover:bg-black text-white rounded-2xl font-black text-lg flex items-center justify-center gap-3 transition-all disabled:opacity-50 disabled:cursor-not-allowed mx-auto shadow-xl shadow-slate-900/20"
            >
              {isActivating ? (
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <h2 className="text-2xl font-black text-slate-800 flex items-center gap-3">
            <div className="relative">
              <UserCheck className="text-primary w-8 h-8 relative z-10" />
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <button 
              ...
              className="w-full md:w-auto py-3 px-6 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-2xl text-sm font-black flex items-center justify-center gap-2 transition-all shadow-sm min-h-[44px]"
            >
              <PowerOff size={18} />
              إيقاف الرادار
            </button>
